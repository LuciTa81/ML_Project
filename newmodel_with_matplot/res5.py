# ============================================
# transforms.RandomRotation(10) ## 수치 20->10도로 변경 ## 크롭된 사진이기에 각도크면 성능저하가능성
# transforms.ColorJitter( ## 추가한 부분 ## 밝기/대비/채도/색조 변형 ## 조명차이 정도로만
# model.fc = nn.Sequential( ## 드롭아웃 추가를 위한 nn.Sequential
#     nn.Dropout(0.5), # fc(Linear)는 완전 연결층이라 과적합이 쉽게 생김 ## 드롭아웃 추가로 방지 및 안정성 강화
...
transform = transforms.Compose([
    ...
    transforms.RandomRotation(10), ## 수치 20->10도로 변경 ## 크롭된 사진이기에 각도크면 성능저하가능성
    transforms.ColorJitter( ## 추가한 부분 ## 밝기/대비/채도/색조 변형 ## 조명차이 정도로만 변형
        brightness=0.2,
        contrast=0.2,
        saturation=0.2,
        hue=0.02 ## 색조는 특히 더 조금
    ),
    transforms.ToTensor(),
...
best_model.fc = nn.Sequential(
    nn.Dropout(0.5), ## 드롭아웃 추가에 따른 수정한 부분
    nn.Linear(in_features, NUM_CLASSES)
)
